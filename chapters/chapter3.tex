...
In seguito alla raccolta di requisiti ho svolto due riunioni con il responsabile tecnico Alex Beggiato e parte dei membri del team JMES. Lo scopo degli incontri era delineare una possibile soluzione ai problemi evidenziati durante l'analisi della precedente versione.\\
...
La soluzione di seguito esposta intende dare un'idea generale di quanto è stato sviluppato nel capitolo \ref{ch:analisi_progettazione_sviluppo}.\\
...
Il punto di partenza è stato fornire ogni PLC di un \gls{thread}. Questo doveva occuparsi dell'interazione con la macchina effettuando le operazioni di base: lettura ad intervalli regolari delle aree di memoria specificate nel file di configurazione e scrittura di valori su richiesta. Da un approfondimento successivo è emersa la possibilità di limitare ancor di più la responsabilità dei singoli \gls{thread}, suddividendo lettura e scrittura ottenendo infine due \gls{thread} per ogni PLC.
